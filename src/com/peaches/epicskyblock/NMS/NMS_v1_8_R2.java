...
public class NMS_v1_8_R2 {
        ...
        for (final Map.Entry<BlockPosition, TileEntity> entry : chunk.getHandle().tileEntities.entrySet()) {
            if (island.isblockinisland(entry.getKey().getX(), entry.getKey().getZ())) {
                ...
                if (tileEntity instanceof TileEntityMobSpawner) {
                    if (EpicSkyBlock.getSkyblock.getConfig().contains("IsTop.Spawners." + ((TileEntityMobSpawner) tileEntity).getSpawner().getMobName().toUpperCase())) {
                        level += EpicSkyBlock.getSkyblock.getConfig().getInt("IsTop.Spawners." + ((TileEntityMobSpawner) tileEntity).getSpawner().getMobName().toUpperCase());
                    }
                } else {
                    if (EpicSkyBlock.getSkyblock.getConfig().contains("IsTop.Blocks." + tileEntity.w().getName().toUpperCase())) {
                        level += EpicSkyBlock.getSkyblock.getConfig().getInt("IsTop.Blocks." + tileEntity.w().getName().toUpperCase());
                    }
                }
            }
        }
    ...
    public static void setBlockSuperFast(int X, int Y, int Z, int blockId, byte data, boolean applyPhysics) {
        net.minecraft.server.v1_8_R2.World w = ((org.bukkit.craftbukkit.v1_8_R2.CraftWorld) EpicSkyBlock.getSkyblock.getWorld()).getHandle();
        ...
        net.minecraft.server.v1_8_R2.IBlockData ibd = net.minecraft.server.v1_8_R2.Block.getByCombinedId(combined);
        if (applyPhysics) {
